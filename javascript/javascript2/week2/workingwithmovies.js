...
const shortTitle = movies.filter(short => short.title.length < 6);
console.log(`Showing the movies name ${shortTitle}`);
console.log(`Number of movies with short title ${shortTitle.length}`);

// 2-  Create an array of movie titles with long movie titles
const longTitle = movies.filter(long => long.title.length > 25);
console.log(`Number of movies with long title ${longTitle}`);

// 3-  Count the number of movies made between 1980-1989 (including both the years)
// Using filter to create a new array with movies made between 1980-1989

const betweenYear = movies.filter(movie => movie.year >= 1980 && movie.year <= 1989
);
console.log(`Presenting movies title made between 1980-1989 ${betweenYear}`);
console.log(`Number of movies made between 1980-1989 ${betweenYear.length}`);

// 4-  Create a new array that has an extra key called tag based on the rating

const rating = movies.map(calledTag => {
if (calledTag.rating >= 7) {calledTag.tag = "Good";} 
else if (calledTag.rating >= 4 && calledTag.rating < 7) {calledTag.tag = "Average";} 
else {calledTag.tag = "Bad";}
return calledTag;});
console.log(`Rating movies ${rating}`);

// 5- filter the movies array to only contain the movies rated higher than 6.

const rating6 = movies.filter(movie => movie.rating > 6).map(movie => movie.rating);
console.log(`the movies rated higher than 6 ${rating6}`);
...
const movies1 = movies.filter(function(movie) {
  if (movie.title.toLowerCase() === "Surfer" || movie.title.toLowerCase() === "Alien" || movie.title.toLowerCase() === "Benjamin") {return true;}
});
console.log(`The total number of movies includes Alien ${movies1.length}`);

const surfer = movies.filter(movie => movie.title.includes("Surfer") || movie.title.includes("Alien") || movie.title.includes("Benjamin"));
console.log(surfer);
console.log(`The total number of movies includes Surfer ${surfer.length}`);

// 7- Create an array of movies where a word in the title is duplicated.
let duplicatedWord = movies.filter(movie => {
let splitted = movie.title.split(" ");for (let i = 0; i < splitted.length; i++) {
...
console.log(duplicatedWord.length);

let total = 0;
let avargeRating;
for (let i = 0; i < movies.length; i++) {total = total + movies[i].rating;avargeRating = (total / movies.length).toFixed(1);}
console.log(total);
...
console.log(rateAverage.toFixed(1));

// 10- Count the total number of Good, Average and Bad movies using reduce.

const ratingGood = rating.reduce((accumulator, currentValue) => accumulator.concat(currentValue), []).filter(item => item.tag === "Good").length;

console.log(`Number of good rating: ${ratingGood}`);

const ratingAverage = rating.reduce((accumulator, currentValue) => accumulator.concat(currentValue), []).filter(item => item.tag === "Average").length;

console.log(`Number of average rating: ${ratingAverage}`);

const ratingBad = rating.reduce((accumulator, currentValue) => accumulator.concat(currentValue), []).filter(item => item.tag === "Bad").length;

console.log(`Number of bad rating: ${ratingBad}`);
